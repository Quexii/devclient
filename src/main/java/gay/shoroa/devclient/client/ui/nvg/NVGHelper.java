...
import gay.shoroa.devclient.client.util.BufferUtil;
...
public class NVGHelper {
    ...
    public static void initFont(String fontName, String fileName) {
        try {
            ByteBuffer data = BufferUtil.get().getResourceBytes(new ResourceLocation("devclient","fonts/"+fileName), 1024);
    ...
    public static void initImage(String imgName, String fileName) {
        try {
            ByteBuffer data = BufferUtil.get().getResourceBytes(new ResourceLocation("devclient","img/"+fileName), 1024);
            LinkedStorage.put(data);
